...
- rona 도구(`get_practice`/`request_skill`)를 호출할 때 인자에 `launcherVersion: "0.3.3"` 을 함께 넣는다 — 설치 토큰과 별개의 부가 인자다(토큰 없이 `get_practice` 를 부를 때도 이 값은 넣는다). 서버가 런처가 최신인지 확인하는 용도다.
